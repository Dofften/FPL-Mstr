...
async function getTopManagersData(x: number) {
  const res = await fetch(`http://localhost:3000/api/fpl/${x}/top250`);
  // The return value is *not* serialized
  // You can return Date, Map, Set, etc.
...
export default async function page({ params }: { params: { slug: number } }) {
  ...
  const top250 = await getTopManagersData(params.slug);
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        >
          <div className="flex justify-around">
            {data
              .filter((player: any) => player.element_type === 1)
              .map((player: any) => (
                <PlayerItem
          ...
          <div className="flex justify-around">
            {data
              .filter((player: any) => player.element_type === 2)
              .map((player: any) => (
                <PlayerItem
                  elementName={player.web_name}
                  team={player.team_name}
          ...
          <div className="flex justify-around">
            {data
              .filter((player: any) => player.element_type === 3)
              .map((player: any) => (
                <PlayerItem
                  elementName={player.web_name}
                  team={player.team_name}
          ...
          <div className="flex justify-around">
            {data
              .filter((player: any) => player.element_type === 4)
              .map((player: any) => (
                <PlayerItem
                  elementName={player.web_name}
                  team={player.team_name}
